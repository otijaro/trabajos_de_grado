...
for j=1:(spf-spi)+1
    ...
    for i=2:f
        tfA(i,:,j)=fft(Ir(i,:,j)); % Transformada de Fourier de las filas de la imagen
        [maxv,index(i,j)]=max(abs(tfA(i,10:round(length(tfA(i,:,j))/2-10))));  %Búsqueda de la frecuencia fundamental
        index(i,j)=index(i,j)+9;
        angletFA(i,j)=angle(tfA(i,index(i,j),j));%Se encuentra la fase inicial
...
for j=1:(spf-spi)+1
    ...
    %Intensidad en nivel de gris de la imagen en la camara
        Ing(j)=sum(sum(I2(:,:,j)));
    %Intensidad real
            fid = fopen('D:\Trabajo tesis\algoritmos\Imagenes\prueba_31_03\amb\PM1918000_amb.txt');
            tline = fgetl(fid);
            c=0;
            while tline > 0 
               c=c+1;
...
save('D:\Trabajo tesis\ALEXANDER\proceso_atm_congelada.mat')
%%clc
clear all
close all
